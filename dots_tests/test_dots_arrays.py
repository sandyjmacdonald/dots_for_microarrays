...
def test_probenames():
    assert_equals(sorted(array.get_probenames()),
                  sorted([feat['ProbeName'] for feat in list(values.values())]))

def test_genenames():
    assert_equals(sorted(array.get_genenames()),
                  sorted([feat['GeneName'] for feat in list(values.values())]))

def test_systematicnames():
    assert_equals(sorted(array.get_systematicnames()),
                  sorted([feat['SystematicName'] for feat in list(values.values())]))

def test_descriptions():
    assert_equals(sorted(array.get_descriptions()),
                  sorted([feat['Description'] for feat in list(values.values())]))

def test_intensities():
    assert_equals(sorted(array.get_intensities()),
                  sorted([float(feat['gProcessedSignal']) for feat in list(values.values())]))

def test_normalisation():
    intensities = [float(feat['gProcessedSignal']) if float(feat['gProcessedSignal']) > 1.0 else 1.0 for feat in list(values.values())]
...
def test_get_groups():
    ...
    assert_equals(sorted(groups), sorted(experiment.get_groups()))
